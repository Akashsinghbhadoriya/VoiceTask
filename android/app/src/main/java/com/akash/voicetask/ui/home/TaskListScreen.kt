...
import com.akash.voicetask.notification.ReminderAlarmReceiver
...
fun TaskListScreen(
    ...
    val prefs = remember {
        context.getSharedPreferences(ReminderAlarmReceiver.PREFS_NAME, android.content.Context.MODE_PRIVATE)
    }
    var soundAlertEnabled by remember {
        mutableStateOf(prefs.getBoolean(ReminderAlarmReceiver.KEY_SOUND_ALERT_ENABLED, true))
    }
                ...
                Column {
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            Text(
                                text = "Sound alert when task is due",
                                style = MaterialTheme.typography.bodyMedium
                            )
                            Text(
                                text = "Play ringtone and show full-screen prompt",
                        ...
                        Switch(
                            ...
                            onCheckedChange = { enabled ->
                                ...
                                prefs.edit()
                                    .putBoolean(ReminderAlarmReceiver.KEY_SOUND_ALERT_ENABLED, enabled)
                                    .apply()
                            }
                        )
                    }
                    ...
                    Text(
                        text = "When off, only a silent notification is shown.",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant
                    )
                }
